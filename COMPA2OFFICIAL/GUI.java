...
public abstract class GUI {
	//filters
	protected abstract void filterByTime();

	protected abstract void unfilterByTime();

	protected abstract void filterNotForCar();

	protected abstract void unfilterNotForCar();

	protected abstract void filterNotForPede();

	protected abstract void unfilterNotForPede();

	protected abstract void filterNotForBicy();

	protected abstract void unfilterNotForBicy();
	...
	/**
	 * Is called when the mouse is right clicked (actually, when the mouse is released),
	 * and is passed the MouseEvent object for that click.
	 */
	protected abstract void onRightClick(MouseEvent e);

	/**
	 * Is called when the mouse is right clicked
	 */
	...
	private void initialise() {
		...
		JButton ArticulationPoint1 = new JButton("Articulation");
		ArticulationPoint1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				Graph.articulationState = !Graph.articulationState;
				redraw();
			}
		});
		...
		oneWay.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				Graph.onewayState = !Graph.onewayState;
				redraw();
			}
		});

		JButton Time = new JButton("Filter by Time");
		Time.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				Graph.preferTime = !Graph.preferTime;
				System.out.print(Graph.preferTime);
				if (Graph.preferTime == true) {
					filterByTime();
				} else {
					unfilterByTime();
				}
		...
		JButton Bike = new JButton("Remove route inaccessible to Bikes");
		Bike.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				Graph.noBike = !Graph.noBike;
				if (Graph.noBike == true) {
					filterNotForBicy();
				} else {
					unfilterNotForBicy();
				}
		...
		JButton Walk = new JButton("Remove route inaccessible to Pedestrians");
		Walk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				Graph.noPedestrian = !Graph.noPedestrian;
				if (Graph.noPedestrian == true) {
					filterNotForPede();
				} else {
					unfilterNotForPede();
				}
		...
		JButton Car = new JButton("Remove route inaccessible to Car");
		Car.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				Graph.noCar = !Graph.noCar;
				if (Graph.noCar == true) {
					filterNotForCar();
				} else {
					unfilterNotForCar();
		...
		loadquit.setLayout(new GridLayout(6, 1));
		...
		loadquit.add(load);
		loadquit.add(quit);
		loadquit.add(ArticulationPoint1);
		loadquit.add(oneWay);
		loadquit.add(Time);
		loadquit.add(Bike);
		loadquit.add(Walk);
